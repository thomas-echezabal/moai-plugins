...
5. Run `node scripts/test-inbox-assistant-hook.mjs` when Inbox Assistant changes.
6. Run `claude plugin validate . --strict`.
7. Validate the Codex manifest with the plugin-creator validator and perform an isolated Codex marketplace install.
8. Open a pull request and merge only after the required validation check passes.
